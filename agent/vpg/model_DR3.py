    ...
    def forward(self, input_color_data, input_depth_data, is_volatile=False, specific_rotation=-1):
        ...
        if is_volatile:
            with torch.no_grad():
                output_prob = []
                interm_feat = []
                push_features_list = []     # For DR3
                grasp_features_list = []    # For DR3
                ...
                for rotate_idx in range(self.num_rotations):
                    ...
                    # For DR3
                    push_features = self.pushnet(interm_push_feat)
                    push_features_list.append([push_features[0]])
                    grasp_features = self.graspnet(interm_grasp_feat)
                    grasp_features_list.append([grasp_features[0]])

                    # Forward pass through branches, undo rotation on output predictions, upsample results
                    output_prob.append([F.interpolate(F.grid_sample(push_features, flow_grid_after, mode='nearest', align_corners=True),
                                                      scale_factor=16, mode='bilinear', align_corners=True),
                                        F.interpolate(F.grid_sample(grasp_features, flow_grid_after, mode='nearest', align_corners=True),
                                                      scale_factor=16, mode='bilinear', align_corners=True)])
                    
            return output_prob, interm_feat, push_features_list, grasp_features_list
            ...
            # Forward pass through branches, undo rotation on output predictions, upsample results
            self.output_prob.append([F.interpolate(F.grid_sample(push_features, flow_grid_after, mode='nearest', align_corners=True),
                                                   scale_factor=16, mode='bilinear', align_corners=True),
                                     F.interpolate(F.grid_sample(grasp_features, flow_grid_after, mode='nearest', align_corners=True),
                                                   scale_factor=16, mode='bilinear', align_corners=True)])

            return self.output_prob, self.interm_feat, push_features, grasp_features