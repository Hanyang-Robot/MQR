...
class VPG():
    def __init__(self, config):
        ...
        # else:
        self.explore_probability = config['explore_probability']
        self.explore_probability_decay = config['explore_probability_decay']
    ...
    def forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1, use_target=False):
        ...
        return push_q_maps, grasp_q_maps, state_feat
    ...
    def get_action(self, state):
        ...
        push_q_maps, grasp_q_maps, state_feat = self.forward(color_heightmap, depth_heightmap, is_volatile=True)
        ...
        if best_push_q_value > best_grasp_q_value:
            ...
            action_type = 'grasp'
        exploration = np.random.uniform() < self.explore_probability
        if exploration: # Exploitation (do best action) vs exploration (do other action)
            action_type = 'push' # if np.random.randint(2) == 0 else 'grasp'
            print(f'Action type: {action_type}(explore) (exploration probability: {self.explore_probability:.5f})')
        else:
            print(f'Action type: {action_type}(exploit) (exploration probability: {self.explore_probability:.5f})')
        self.is_exploit_log.append([0 if exploration else 1])
        self.logger.log('is_exploit', self.is_exploit_log)
    ...
    def train(self, state, action, reward, next_sate):
        ...
        label_value = self.get_label_value(reward, next_color_heightmap, next_depth_heightmap)
        ...
        if not self.is_testing:
            self.explore_probability = max(0.5 * np.power(0.9999, self.step), 0.1) if self.explore_probability_decay else 0.5

        # Do sampling for experience replay
        sample_action_type = action_type
        if sample_action_type == 'push':
            sample_action_type_id = 0
            sample_reward = 0 if reward == 0.5 else 0.5
        elif sample_action_type == 'grasp':
            sample_action_type_id = 1
            sample_reward = 0 if reward == 1 else 1

        # Get samples of the same motion action but with different results
        sample_index = np.argwhere(np.logical_and(np.asarray(self.reward_value_log)[1:self.step, 0] == sample_reward, np.asarray(self.executed_action_log)[1:self.step, 0] == sample_action_type_id))

        if sample_index.size > 0:
            # Find sample with highest surprise value
            sample_surprise_values = np.abs(np.asarray(self.q_value_log)[sample_index[:, 0]] - np.asarray(self.label_value_log)[sample_index[:, 0]])
            sorted_surprise_index = np.argsort(sample_surprise_values[:, 0])
            sorted_sample_index = sample_index[sorted_surprise_index, 0]
            pow_law_exp = 2
            rand_sample_index = int(np.round(np.random.power(pow_law_exp, 1)*(sample_index.size - 1)))
            sample_step = sorted_sample_index[rand_sample_index]
            print(f'Experience replay: step {sample_step} (surprise value: {sample_surprise_values[sorted_surprise_index[rand_sample_index]][0]:.3f})')
            ...
            with torch.no_grad():
                sample_push_q_maps, sample_grasp_q_maps, sample_state_feat = self.forward(sample_color_heightmap, sample_depth_heightmap, is_volatile=True)

            # Get labels for sample and backpropagate
            sample_best_pix_index = (np.asarray(self.executed_action_log)[sample_step, 1:4]).astype(int)
            per_loss_value = self.backpropagate(sample_color_heightmap, sample_depth_heightmap, sample_action_type, sample_best_pix_index, self.label_value_log[sample_step])
            self.per_loss_value_log.append([per_loss_value])
            print(f"per_loss_value is logging now")
            self.logger.log('per_loss_value', self.per_loss_value_log)
    ...
    def get_label_value(self, reward, next_color_heightmap, next_depth_heightmap):
        ...
        if reward == 0:
            ...
            next_push_q_maps, next_grasp_q_maps, next_state_feat = self.forward(next_color_heightmap, next_depth_heightmap, is_volatile=True, use_target=True)
            next_q_value = max(np.max(next_push_q_maps), np.max(next_grasp_q_maps))
    ...
    def backpropagate(self, color_heightmap, depth_heightmap, action_type, best_pix_index, label_value):
        ...
        if action_type == 'push':
            ...
            push_q_map, grasp_q_map, state_feat = self.forward(color_heightmap, depth_heightmap, specific_rotation=best_pix_index[0])
            ...
            push_q_map, grasp_q_map, state_feat = self.forward(color_heightmap, depth_heightmap, specific_rotation=opposite_rotate_idx)
            loss = self.criterion(self.model.output_prob[0][1].view(1, 320, 320), torch.from_numpy(label).float().to(self.device)) * torch.from_numpy(label_weights).float().to(self.device)
            loss = loss.sum()
            loss.backward()
            loss_value += loss.cpu().data.numpy()
    ...
    def load_model(self, model_weight):
        self.model.load_state_dict(torch.load(self.logger.models_directory+model_weight))
        print(f'Load trained model: {model_weight}')

    # Load pre-trained model weight
    def load_model_sim_weights(self, model_weight):
        self.model.load_state_dict(torch.load(model_weight))
        print(f'Load trained model: {model_weight}')

    # Pre-load execution info and RL variables
    def preload(self):
        print(f"transitions_directory : {self.logger.transitions_directory}")

        self.executed_action_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'executed_action.txt'), delimiter=' ')
        self.iteration = self.executed_action_log.shape[0]
        self.step = self.iteration
        self.executed_action_log = self.executed_action_log[0:self.iteration,:]
        self.executed_action_log = self.executed_action_log.tolist()

        self.q_value_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'q_value.txt'), delimiter=' ')
        ...
        self.reward_value_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'reward_value.txt'), delimiter=' ')
        self.reward_value_log = self.reward_value_log[0:self.iteration]
        self.reward_value_log.shape = (self.iteration,1)
        self.reward_value_log = self.reward_value_log.tolist()

        self.label_value_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'label_value.txt'), delimiter=' ')
        ...
        self.clearance_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'clearance.txt'), delimiter=' ')
        self.clearance_log.shape = (self.clearance_log.shape[0],1)
        self.clearance_log = self.clearance_log.tolist()

        self.is_exploit_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'is_exploit.txt'), delimiter=' ')
        ...
        self.loss_value_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'loss_value.txt'), delimiter=' ')
        self.loss_value_log = self.loss_value_log[0:self.iteration]
        self.loss_value_log.shape = (self.iteration,1)
        self.loss_value_log = self.loss_value_log.tolist()

        self.per_loss_value_log = np.loadtxt(os.path.join(self.logger.transitions_directory, 'per_loss_value.txt'), delimiter=' ')
        self.per_loss_value_log = self.per_loss_value_log[0:len(self.per_loss_value_log)]
        self.per_loss_value_log.shape = (len(self.per_loss_value_log),1)
        self.per_loss_value_log = self.per_loss_value_log.tolist()