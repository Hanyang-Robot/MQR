...
from .logger_offline_RL import Logger_for_offline_RL
...
class MQR():
    def __init__(self, config):
        ...
        if self.is_testing:
            ...
            self.logger = Logger_for_offline_RL(config)
    ...
    def train_offline(self):
        ...
        label_value = self.get_label_value(sample_reward, next_sample_color_heightmap, next_sample_depth_heightmap)
        loss_total, loss_TD, loss_CQL = self.backpropagate_offline(sample_color_heightmap, sample_depth_heightmap, sample_action_type, sample_best_pix_index, label_value)
    ...
    def backpropagate_offline(self, color_heightmap, depth_heightmap, action_type, best_pix_index, label_value):
        # Compute labels
        label = np.zeros((1, 320, 320))
        action_area = np.zeros((224, 224))
        action_area[best_pix_index[1]][best_pix_index[2]] = 1
        ...
        if action_type == 'push':
            ...
            # Backward
            loss_total_p = loss_TD_p + loss_CQL_p
            ...
            # Backward
            loss_total_n = loss_TD_n + loss_CQL_n
        ...
        print(f'Training loss: {loss_total:.3f}, CQL loss: {loss_CQL:.3f}, TD loss: {loss_TD:.3f}')
        self.optimizer.step()

        return loss_total, loss_TD, loss_CQL
