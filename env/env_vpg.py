...
from env.robot import UR5e
from env.gripper import Robotiq2F85
from env.utils import sample_pose

# TODO: step_with_rendering에 save_video 옵션 만들기
# fourcc = cv2.VideoWriter_fourcc(*'mp4v')
# video = cv2.VideoWriter('video.mp4', fourcc, 100.0, (1024, 768))
# video.release()
...
class Grasping(control.Task):
    ...
    def __init__(self, physics, config):
        ...
        self.grasping_area_size = physics.named.model.site_size['grasping_area']
        ...
        self.workspace = [(self.grasping_area_position[i] - self.grasping_area_size[i],
                           self.grasping_area_position[i] + self.grasping_area_size[i]) for i in range(2)]
    ...
    def after_step(self, physics):
        ...
        self.robot.set(physics, self.robot.HOME_JOINT_POSITION)
        self.gripper.set(physics, self.gripper.OPEN_POSITION)
        self.check_outside_objects(physics)
    ...
    def get_termination(self, physics):
        ...
        if sum(self.action_fail_count) >= 10: 
            ...
            self.check_num_of_inside_objects(physics)
            self.check_num_of_outside_obejcts(physics)

            return True

        for object_id in range(self.num_objects):
            object_position = physics.named.data.qpos[f'object_{object_id}/'][0:3].copy()
            if ((self.grasping_area[0][0] < object_position[0] < self.grasping_area[0][1]) and
    ...
    def check_num_of_inside_objects(self, physics):
        for object_id in range(self.num_objects):
            object_position = physics.named.data.qpos[f'object_{object_id}/'][0:3].copy()
            if ((self.grasping_area_for_check[0][0] < object_position[0] < self.grasping_area_for_check[0][1]) and
                (self.grasping_area_for_check[1][0] < object_position[1] < self.grasping_area_for_check[1][1])):
                self.num_of_inside_objs+=1

    def check_num_of_outside_obejcts(self, physics):
        self.num_of_outside_objs_by_push = len(self.list_of_outside_objs_by_push)
        self.num_of_outside_objs_by_grasp = len(self.list_of_outside_objs_by_grasp)

    def check_outside_objects(self, physics):
        for object_id in range(self.num_objects):
            object_position = physics.named.data.qpos[f'object_{object_id}/'][0:3].copy()
            if not((self.grasping_area_for_check[0][0] < object_position[0] < self.grasping_area_for_check[0][1]) and \
                (self.grasping_area_for_check[1][0] < object_position[1] < self.grasping_area_for_check[1][1])) and \
               not((self.placing_area_for_check[0][0] < object_position[0] < self.placing_area_for_check[0][1]) and \
                (self.placing_area_for_check[1][0] < object_position[1] < self.placing_area_for_check[1][1])):
                if self.action[0] == 'push': 
                    if object_id in self.list_of_outside_objs_by_push or object_id in self.list_of_outside_objs_by_grasp:
                        pass
                    else:
                        self.list_of_outside_objs_by_push = np.append(self.list_of_outside_objs_by_push, object_id)
                        print(f"Object {object_id} is moved out of the workspace by pushing.")
                elif self.action[0] == 'grasp': 
                    if object_id in self.list_of_outside_objs_by_push or object_id in self.list_of_outside_objs_by_grasp:
                        pass
                    else:
                        self.list_of_outside_objs_by_grasp = np.append(self.list_of_outside_objs_by_grasp, object_id)
                        print(f"Object {object_id} is moved out of the workspace by grasping")
    ...
    def preprocess_action(self, physics, action):
        ...
        # Compute 3D position of pixel, {c}와 {w}는 y=x 대칭
